...
/*   Updated: 2023/05/13 08:29:15 by eguefif          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_image_on_image(t_screen_buffer *dst, t_image src, int x, int y)
{
	...
	{
		cols = 0;
		while (cols < src.width)
		{
			position_dst = floor(cols + x) * dst->bits_per_pixel / 8 + floor(y + row) * dst->size_line;
			position_src = cols * src.bits_per_pixel / 8 + row * src.size_line;
			memcpy((dst->start_area_ptr + position_dst), src.start_area_ptr+ position_src, 4 * sizeof(char));
			cols++;
		}
		row++;
	}
